...
export async function navigate(
  ...
  try {
    ...
    if (action === 'back') {
      ...
      // 先注册监听，再发命令
      console.error('[debug] registering Page.loadEventFired listener');
      const loadPromise = context.waitForEvent(ws, 'Page.loadEventFired');
      console.error('[debug] sending Page.navigate');

      await context.sendCommand(ws, 'Page.navigate', { url: action });
      console.error('[debug] waiting for loadEventFired...');


      await loadPromise;
      console.error('[debug] loadEventFired received!');
